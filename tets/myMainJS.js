    ...
    $.ajax({
            ...
            success: function(response) {
              ...
              var textTagMusicList = ''
                  ...
                  {
                    textTagMusicList = textTagMusicList +'<div id="row_playlist"><div id="row_playlist_{3}" url={2}><div id="playlist_content"><button class="but_" id="but_{3}" onclick="play_choose({2}, {3})"><a class="chars_playlist_{3}">▶</a></button></div><div id="text_song_{3}">{0} - {1}</div></div></div>'.f(music_dict[_][0], music_dict[_][1], "'{0}'".f(music_dict[_][2]), _);
                  }
    ...
    if(text.length == 78 ){
        $.ajax({
            ...
            success: function(response) {
              ...
              {
                textTagGameList = textTagGameList + '<ul style="list-style-type:circle"><li><button onclick="pasteUrl({1})">{0}</button></li></ul>'.f(gaMe, "'{0}'".f(dict['game_list'][gaMe]));
              }
